...
<h1 align="center">Креатива в AI нет.<br>Есть методологии победителей.</h1>

<p align="center"><sub>There is no creativity in AI. There are the methodologies of winners.</sub></p>
...
<a href="https://cd-skill.com">
  <img src="assets/cd-skill-site.webp" alt="cd-skill.com — подробный разбор метода, все 20 техник и пайплайн" width="100%">
</a>
...
<img src="assets/pipeline-phases.webp" alt="Пайплайн: 5 фаз от брифа до идеи — intake, insight, ideas, evaluate, articulate; фаза 04 крутится циклом, пока идея не станет сильной" width="100%">
...
<img src="assets/why-averaged.webp" alt="Модель возвращает усреднённое и заезженное — без инсайта и структуры AI работает как конвейер: штампует одно и то же" width="100%">
...
<img src="assets/case-library.webp" alt="571 награждённый кейс внутри — каждая карточка разбирает инсайт, исполнение, почему сработало и что украсть как метод" width="100%">
...
<img src="assets/methods-20.webp" alt="20 методов идеации в 6 группах — структурные, ассоциация, инверсия, возмущение, объём, бонус; на каждый бриф берётся по технике из разных групп" width="100%">
...
<img src="assets/scoring-loop.webp" alt="6 взвешенных критериев — originality 0.25, strategic fit 0.20, emotion 0.20, craftability 0.15, scalability 0.10, simplicity 0.10; дальше проходит только идея с 9 из 10 и human impact 7+" width="100%">
...
Через [skills.sh](https://skills.sh/smixs/creative-director-skill) - ставит в любой из 70 с лишним агентов:

```bash
npx skills add smixs/creative-director-skill        # спросит, куда ставить
npx skills add smixs/creative-director-skill -g     # глобально, для всех проектов
...
Этот скилл отдаёт идею и сценарий. Превратить их в кадры и клипы умеет соседний репо **[visual-skills](https://github.com/smixs/visual-skills)** - два скилла, которые подхватывают там, где этот заканчивает. `image` пишет промпты уровня арт-директора для кейфреймов, character sheets и раскадровок (Nano Banana, GPT Image 2). `video` превращает их в режиссёрские промпты для Seedance, Kling и Veo. Полная цепочка: **идея и сценарий (этот скилл) → кейфреймы (`image`) → движение (`video`)**.
